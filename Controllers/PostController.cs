...
namespace Blog.Controllers
{
    ...
    public class PostController : ControllerBase
    {
        ...
        public async Task<IActionResult> DetailsAsunc([FromServices] BlogDataContext context, [FromRoute] int id)
        {
            try
            {
                var posts = await context
                ...
                if (posts == null) return NotFound(new ResultViewModel<Post>("Não encontrado"));

                return Ok(new ResultViewModel<Post>(posts));
            }
            catch
            {
                return StatusCode(500, new ResultViewModel<Post>("05X04 - Falha interna"));
            }
        }
